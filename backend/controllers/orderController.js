...
const updateOrderToPaid = asyncHandler(async (req, res) => {
  ...
  if (order) {
    order.isPaid = true;
    order.paidAt = Date.now(); // Invoke Date.now as a function
    order.paymentResult = {
      cardNumber,
      cvv,
      // You can add more payment result details here if needed
    };
...
const updateOrderToDelivered = asyncHandler(async (req, res) => {
  ...
  if (order) {
    ...
    order.deliveredAt = Date.now(); // Invoke Date.now as a function
